...
@profile_bp.route("/memories", methods=["GET"])
def get_memories():
    """获取用户的记忆列表。"""
    user_id = request.headers.get("X-User-Id")
    if not user_id:
...
@profile_bp.route("/memories", methods=["POST"])
def add_memory():
    """主动添加一条新记忆。"""
    user_id = request.headers.get("X-User-Id")
    if not user_id:
    ...
    try:
        ...
        db.add(new_memory)
        db.commit()
        return jsonify({"success": True, "memory": new_memory.to_dict()})
    except Exception as e:
        db.rollback()
        return jsonify({"error": str(e)}), 500
    finally:
...
@profile_bp.route("/memories/<memory_id>", methods=["DELETE"])
def delete_memory(memory_id):
    ...
    try:
        from models.profile import KeyMemory
        memory = db.query(KeyMemory).filter_by(id=memory_id, user_id=user_id).first()
        if not memory:
