...
class PromptEngine:
    ...
    def get_system_prompt(
        ...
        if shade_name == "mirror_test":
            return base_prompt + "\n\n## 镜像测试特别要求\n你现在正在进行“深层自我对谈（镜像测试）”。作为 {user_name} 的数字分身，你的目标是挖掘尚未被捕获的人格特征。请主动引导话题，通过提问或反思来深入聊聊 {user_name} 的内心想法。如果话题有些干瘪或冷场，你应该更加主动地抛出引发深度思考的问题。".replace("{user_name}", user.nickname or "他")
